    ...
    <footer style={{ padding: "2.5rem 2.5rem 2rem", borderTop: "1px solid rgba(200,164,110,0.12)" }}>
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", borderTop: "1px solid rgba(245,244,240,0.05)", paddingTop: "1rem", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem" }}>
        ...
        <div style={{ display: "flex", gap: "1.25rem", alignItems: "center" }}>
          <Link href="/legal" style={{ fontFamily: "var(--font-inter)", fontSize: "0.7rem", color: "rgba(245,244,240,0.25)", letterSpacing: "0.04em", textDecoration: "none" }}>Legal</Link>
...
function Hero() {
  return (
    <section style={{ paddingTop: "18rem", paddingBottom: "7rem", paddingLeft: "2.5rem", paddingRight: "2.5rem", borderBottom: "1px solid rgba(245,244,240,0.06)" }}>
      <div style={{ maxWidth: "800px" }}>
        <SectionLabel>About</SectionLabel>
        <h1 style={{ fontFamily: "var(--font-playfair)", fontSize: "clamp(2.2rem, 5.5vw, 4.5rem)", fontWeight: 500, lineHeight: 1.1, color: "var(--offwhite)", marginBottom: "2.5rem", letterSpacing: "-0.01em" }}>
          We build products
          <br />
        ...
        <p style={{ fontFamily: "var(--font-noto)", fontSize: "1.05rem", lineHeight: 2, color: "rgba(245,244,240,0.78)", fontWeight: 300, maxWidth: "600px", letterSpacing: "0.04em", marginBottom: "0.75rem" }}>
          Dexter & Co. は、課題発見からプロダクト化までを一貫して行う課題解決型ブランドです。
        </p>
        <p style={{ fontFamily: "var(--font-noto)", fontSize: "0.95rem", lineHeight: 1.9, color: "rgba(245,244,240,0.62)", fontWeight: 300, maxWidth: "560px", letterSpacing: "0.04em" }}>
  ...
  return (
    <section style={{ padding: "10rem 2.5rem", borderBottom: "1px solid rgba(245,244,240,0.06)", background: "rgba(18,37,57,0.3)" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <h2 style={{ fontFamily: "var(--font-playfair)", fontSize: "clamp(1.35rem, 2.8vw, 2rem)", fontWeight: 500, color: "var(--offwhite)", marginBottom: "3rem", letterSpacing: "0.02em" }}>
          私たちのアプローチ
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "2rem", padding: "2rem 0", borderBottom: "1px solid rgba(245,244,240,0.06)" }}>
              <span style={{ fontFamily: "var(--font-inter)", fontSize: "0.65rem", letterSpacing: "0.22em", color: "var(--gold)", opacity: 0.5, minWidth: "2rem", paddingTop: "0.2rem" }}>
                {String(i + 1).padStart(2, "0")}
              </span>
              <p style={{ fontFamily: "var(--font-noto)", fontSize: "1rem", fontWeight: 300, color: "rgba(245,244,240,0.68)", lineHeight: 1.9, letterSpacing: "0.02em" }}>{item}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section style={{ padding: "10rem 2.5rem", borderBottom: "1px solid rgba(245,244,240,0.06)" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <h2 style={{ fontFamily: "var(--font-playfair)", fontSize: "clamp(1.6rem, 3.5vw, 2.4rem)", fontWeight: 500, fontStyle: "italic", color: "var(--offwhite)", marginBottom: "4rem" }}>
          課題から始める。
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {items.map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "2rem", padding: "2rem 0", borderBottom: "1px solid rgba(245,244,240,0.06)" }}>
              <span style={{ display: "block", width: "6px", height: "6px", borderRadius: "50%", backgroundColor: "var(--gold)", opacity: 0.5, minWidth: "6px", marginTop: "0.6rem" }} />
              <p style={{ fontFamily: "var(--font-noto)", fontSize: "1rem", fontWeight: 300, color: "rgba(245,244,240,0.68)", lineHeight: 1.9, letterSpacing: "0.02em" }}>{item}</p>
            </div>
          ))}
        </div>
...
/* ─── How we build products — NEW ─── */
function HowWeBuildProducts() {
  return (
    <section style={{ padding: "10rem 2.5rem", borderBottom: "1px solid rgba(245,244,240,0.06)", background: "rgba(18,37,57,0.3)" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        <SectionLabel>How we build products</SectionLabel>
        <h2 style={{ fontFamily: "var(--font-playfair)", fontSize: "clamp(1.6rem, 3.5vw, 2.4rem)", fontWeight: 500, color: "var(--offwhite)", marginBottom: "1rem" }}>
          課題からプロダクトへ。
          <span style={{ display: "block", fontFamily: "var(--font-inter)", fontSize: "0.7rem", letterSpacing: "0.18em", color: "rgba(200,164,110,0.45)", marginTop: "0.5rem", fontWeight: 300 }}>From discovery to delivery.</span>
        </h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
          {[
            { step: "01", who: "Dexter & Co.", action: "課題を見つける", desc: "現場・業務・日常を観察し、本質的な問題を特定します。" },
            { step: "02", who: "Dexter & Co.", action: "仕組みに整理する", desc: "課題の構造を整理し、解決策の方向性を設計します。" },
            { step: "03", who: "DexTech", action: "プロダクト化する", desc: "AIツール・業務システム・アプリとして実装します。" },
            { step: "04", who: "Products", action: "Lily / Silent / TabiLogとして届ける", desc: "実際に使われるプロダクトとして、ユーザーの手元へ。" },
          ].map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "2rem", padding: "2.2rem 0", borderBottom: "1px solid rgba(245,244,240,0.06)" }}>
              <span style={{ fontFamily: "var(--font-inter)", fontSize: "0.65rem", letterSpacing: "0.22em", color: "var(--gold)", opacity: 0.5, minWidth: "2rem", paddingTop: "0.25rem" }}>{item.step}</span>
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
                  <p style={{ fontFamily: "var(--font-noto)", fontSize: "1.02rem", fontWeight: 500, color: "rgba(245,244,240,0.88)", letterSpacing: "0.03em" }}>{item.action}</p>
                  <span style={{ fontFamily: "var(--font-inter)", fontSize: "0.6rem", letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--gold)", border: "1px solid rgba(200,164,110,0.28)", padding: "0.15rem 0.6rem", opacity: 0.8 }}>{item.who}</span>
                </div>
                <p style={{ fontFamily: "var(--font-noto)", fontSize: "0.85rem", fontWeight: 300, color: "rgba(245,244,240,0.5)", lineHeight: 1.8, letterSpacing: "0.02em" }}>{item.desc}</p>
              </div>
            </div>
          ))}
        </div>

        {/* 2カラム：Dexter / DexTech の役割説明 */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "2rem", marginTop: "5rem" }}>
          {/* Dexter */}
          <div style={{ padding: "2.5rem 2rem", border: "1px solid rgba(200,164,110,0.2)", background: "rgba(200,164,110,0.04)", display: "flex", flexDirection: "column", gap: "1rem" }}>
            <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.62rem", letterSpacing: "0.24em", textTransform: "uppercase", color: "var(--gold)", opacity: 0.65 }}>The Company</p>
            <p style={{ fontFamily: "var(--font-playfair)", fontSize: "1.3rem", fontWeight: 500, color: "var(--offwhite)" }}>
              DEXTER <span style={{ color: "var(--gold)" }}>&</span> Co.
            </p>
            <span style={{ display: "block", width: "28px", height: "1px", backgroundColor: "var(--gold)", opacity: 0.4 }} />
            <p style={{ fontFamily: "var(--font-noto)", fontSize: "0.88rem", lineHeight: 1.9, color: "rgba(245,244,240,0.62)", fontWeight: 300, letterSpacing: "0.02em" }}>
              課題を見つけ、戦略として整理します。
            </p>
          </div>
          {/* DexTech */}
          <div style={{ padding: "2.5rem 2rem", border: "1px solid rgba(200,164,110,0.12)", background: "rgba(18,37,57,0.5)", display: "flex", flexDirection: "column", gap: "1rem" }}>
            <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.62rem", letterSpacing: "0.24em", textTransform: "uppercase", color: "var(--gold)", opacity: 0.65 }}>The Dev Brand</p>
            <p style={{ fontFamily: "var(--font-playfair)", fontSize: "1.3rem", fontWeight: 500, color: "var(--offwhite)" }}>
              Dex<span style={{ color: "var(--gold)" }}>Tech</span>
            </p>
            <span style={{ display: "block", width: "28px", height: "1px", backgroundColor: "var(--gold)", opacity: 0.4 }} />
            <p style={{ fontFamily: "var(--font-noto)", fontSize: "0.88rem", lineHeight: 1.9, color: "rgba(245,244,240,0.62)", fontWeight: 300, letterSpacing: "0.02em" }}>
              課題を、AIツール・業務システム・アプリとして実装します。
            </p>
          </div>
        </div>
      </div>
...
const products = [
  {
    name: "Lily Series", tag: "Store Operations", index: "01",
  ...
  {
    name: "Silent", tag: "Privacy AI", index: "02",
    description: "履歴を残さない、プライバシー重視のAIチャット。日常の判断や業務のちょっとした意思決定を、安心してサポートします。",
    cta: [{ label: "今すぐ使う", href: "/silent", primary: true }],
  },
  {
    name: "TabiLog", tag: "Travel & Life", index: "03",
    description: "旅の記録と振り返りをまとめて管理できるアプリ。支出・思い出・移動履歴を一元化し、AIが旅の気づきや振り返りをサポートします。",
    cta: [{ label: "アプリを見る", href: "https://tabilog.app", primary: true }],
  },
];

function Products() {
  return (
    <section style={{ padding: "10rem 2.5rem", borderBottom: "1px solid rgba(245,244,240,0.06)", background: "rgba(18,37,57,0.2)" }}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          {products.map((product) => (
            <div key={product.name} style={{ display: "grid", gridTemplateColumns: "3rem 1fr", gap: "1.5rem", padding: "2.5rem 0", borderBottom: "1px solid rgba(245,244,240,0.06)", alignItems: "start" }}>
              <p style={{ fontFamily: "var(--font-inter)", fontSize: "0.65rem", letterSpacing: "0.22em", color: "var(--gold)", opacity: 0.5, paddingTop: "0.3rem" }}>{product.index}</p>
              <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
                  <h3 style={{ fontFamily: "var(--font-playfair)", fontSize: "1.25rem", fontWeight: 500, color: "var(--offwhite)" }}>{product.name}</h3>
                  <span style={{ fontFamily: "var(--font-inter)", fontSize: "0.62rem", letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--gold)", border: "1px solid rgba(200,164,110,0.3)", padding: "0.15rem 0.65rem" }}>{product.tag}</span>
                </div>
                <p style={{ fontFamily: "var(--font-noto)", fontSize: "0.9rem", lineHeight: 1.9, color: "rgba(245,244,240,0.68)", fontWeight: 300, maxWidth: "560px", letterSpacing: "0.03em" }}>{product.description}</p>
                <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap", marginTop: "0.5rem" }}>
                  {product.cta.map((btn, j) => (
                    <a key={j} href={btn.href} target={btn.href.startsWith("http") ? "_blank" : undefined} rel={btn.href.startsWith("http") ? "noopener noreferrer" : undefined}
                      style={{ display: "inline-block", padding: "0.65rem 1.6rem", fontFamily: "var(--font-noto)", fontSize: "0.78rem", fontWeight: 400, letterSpacing: "0.05em", textDecoration: "none", transition: "all 0.25s", borderRadius: "8px", ...(btn.primary ? { backgroundColor: "var(--gold)", color: "var(--navy)" } : { border: "1px solid rgba(200,164,110,0.4)", color: "var(--gold)" }) }}
                      onMouseEnter={(e) => { if (btn.primary) e.currentTarget.style.opacity = "0.85"; else e.currentTarget.style.borderColor = "rgba(200,164,110,0.8)"; }}
                      onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; if (!btn.primary) e.currentTarget.style.borderColor = "rgba(200,164,110,0.4)"; }}>
                      {btn.label}
                    </a>
                  ))}
                </div>
        ...
        {/* CTA */}
        <div style={{ marginTop: "4rem", padding: "2.5rem 2rem", border: "1px solid rgba(200,164,110,0.2)", background: "rgba(200,164,110,0.04)" }}>
          <p style={{ fontFamily: "var(--font-noto)", fontSize: "0.95rem", lineHeight: 1.9, color: "rgba(245,244,240,0.68)", fontWeight: 300, marginBottom: "1.5rem", letterSpacing: "0.03em" }}>
            プロダクト開発・業務改善のご相談は、お気軽にどうぞ。
          </p>
          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
            <a href="https://line.me/ti/p/~hello-dexter" target="_blank" rel="noopener noreferrer" className="btn-gold">LINEで相談する</a>
            <a href="/contact" className="btn-ghost">お問い合わせ</a>
          </div>
        </div>
      </div>
    </section>
